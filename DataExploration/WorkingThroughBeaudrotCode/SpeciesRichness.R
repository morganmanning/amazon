### ALL OF THIS IS EDITED FROM LYDIA BEAUDROT https://github.com/lbeaudrot/Species-Richness/blob/master/File_Overview ###
...
pantheria <- read.delim("PanTHERIA_1-0_WR05_Aug2008.txt", header = TRUE, sep = "\t") # PanTHERIA data
#pantheria$Binomial <- pantheria$MSW05_Binomial
...
mammalianTraits <- cbind(mammals[1:4], 
                         mammals$AdultHeadBodyLen_mm, 
                         mammals$LitterSize, 
                         mammals$GR_Area_km2, 
                         as.factor(mammals$ActivityCycle), 
                         as.factor(mammals$HabitatBreadth), 
                         as.factor(mammals$DietBreadth), 
                         mammals$Guild)
...
mammalianTraits$Family <- as.factor(mammalianTraits$Family)
...
# Manuall add "-" to "VB-" Site.Code so that later merges will operate correctl